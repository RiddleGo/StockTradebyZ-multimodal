...
class BaseReviewer:
    ...
    def run(self):
        ...
        suggestion = self.generate_suggestion(
            pick_date=pick_date, all_results=all_results, min_score=min_score
        )
        ...
        if self.config.get("export_html"):
            try:
                from export_review_html import export_to_html
            except ImportError:
                from agent.export_review_html import export_to_html
            out_html = export_to_html(
                suggestion_path=suggestion_file,
                out_dir=out_dir,
                kline_dir=self.kline_dir,
                output_path=out_dir / "report.html",
                embed_images=self.config.get("embed_report_images", True),
                open_browser=self.config.get("open_report", False),
            )
            print(f"[INFO] HTML 报告已生成: {out_html}")
        print("\n✅ 全部完成。")
